...
func InstallGenFile(currentdir string) {
    genfile_file, err := os.Create(currentdir+"/sys/0x0001.base/genfile/tasks/main.yml") 
    CheckErr(err)
...
func InstallIpvsYaml(currentdir string, master_array []string) {
    ...
    ipvsinityaml_file, err := os.Create(currentdir+"/sys/0x0002.cfssl/copycfssl/templates/ipvsinit_ep.yaml") 
    CheckErr(err)
