...
func InstallYML(mode string, softDir string, currentDir string, currentUser string, logName string, upgradeKernel string, osTypeResult string, cniPlugin string) {
    var osCompatibilityLayout,cniPluginStr string
...
func OnemasterInstallYML(mode string, softDir string, currentDir string, currentUser string, logName string, upgradeKernel string, osTypeResult string, cniPlugin string) {
    var osCompatibilityLayout,cniPluginStr string
    if upgradeKernel  == "yes" { 
        if osTypeResult == "centos7" || osTypeResult == "rhel7" {
            osCompatibilityLayout = "- remote_user: root\n  hosts: node\n  gather_facts: no\n  roles:\n    - "+softDir+"/sys/0x0000000000base/kernel\n"
