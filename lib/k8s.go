...
func GetNodeInfo(label string, nodeIP string, currentDir string, logName string, mode string) (string,string,string,string,string,string,string) {
    _,_,_,subProcessDir,_ := ParameterConvert(mode, "", "", "", label, "")
    kubecfg,err := ReadFile(currentDir+"/data/output"+subProcessDir+"/cert/ssl/kube-install.kubeconfig")
    if kubecfg == "" || err != nil {
        return "","","","","","",""
        ...
        if infoConditionsLen > 0 {
            runStatus = fmt.Sprintf("%s", info.Status.Conditions[infoConditionsLen-1].Type)
        }
        runC := info.Status.NodeInfo.ContainerRuntimeVersion
        ...
        return runStatus,runC,osVer,kernelVer,cpu,memory,createTime
    }
}
