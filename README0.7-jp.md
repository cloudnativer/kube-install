...
注意：Githubにパッケージされているので、ローカルのネットワーク環境があまり良くない場合は、断続的な継続をサポートするダウンロードソフトを使ってパッケージをダウンロードすることをお勧めします。
...
## 2.4 ワンタッチで配置kubernetesクラスタをインストールします。
...
## 2.5 Kubernetes dashboard画面に登録する
...
## kube-installのWeb管理サービスを実行します

まず、`kube-install -init`コマンド初期化システム環境を実行して（前に初期化したらスキップできます）、そして`systemctl start kube-install`コマンドを実行して、kube-installのWeb管理プラットフォームサービスを実行してください。
...
# [4] 拡張容量と廃棄Node|修復Master|アンインストールクラスタ
