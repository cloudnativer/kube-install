二進位管道一鍵離線安裝高可用的kubernetes集羣，同時支持添加kubernetes節點、删除kubernetes節點、銷毀kubernetes主機、重建kubernetes主機、卸載集羣等。
...
首先，你需要執行`kube-install -init`命令初始化系統環境（如果前面已經初始化過了可以跳過），然後執行`systemctl start kube-install`命令來運行kube-install的Web管理平臺服務。
...
除了使用`kube-install -exec addnode`命令進行k8s-node節點擴容外，你也同樣可以使用kube-install的Web管理平臺來對k8s-node節點進行擴容。<a href=“docs/webinstall0.7-zh.md”>點擊這裡可以查看使用kube-install的Web管理平臺來擴容k8s-node節點的方法</a>。
